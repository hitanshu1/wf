...
class MinimapPainter<T> extends CustomPainter {
  ...
  void _drawConnections(Canvas canvas, double scale) {
    if (controller.connections.isEmpty) return;

    final connectionPaint = Paint()
      ..color = nodeColor.withValues(alpha: 0.35)
      ..style = PaintingStyle.stroke
      ..strokeWidth = math.min(1.5, math.max(0.5, 1 / scale));
    ...
    for (final connection in controller.connections) {
      ...
      final sourcePosition = sourceNode.position.value;
      final sourceSize = sourceNode.size.value;
      final targetPosition = targetNode.position.value;

...
class NodeFlowEditor<T> extends StatefulWidget {
  ...
  /// Called when the canvas enters or exits a pan/drag gesture.
  ///
  /// Receives `true` when panning starts and `false` when it ends.
  final ValueChanged<bool>? onCanvasPanStateChanged;
...
class _NodeFlowEditorState<T> extends State<NodeFlowEditor<T>>
  ...
  void initState() {
    ...
    final initialMatrix = Matrix4.identity()
      ..translate(viewport.x, viewport.y, 0)
  ...
  void _setCanvasPanState(bool isPanning) {
    if (_isCanvasPanning == isPanning) return;
    _isCanvasPanning = isPanning;

    if (isPanning) {
      widget.controller._updateInteractionState(
        cursor: widget.theme.dragCursorStyle,
      );
    } else if (widget.controller.draggedNodeId == null &&
        !widget.controller.isConnecting &&
        !widget.controller.isDrawingSelection) {
      widget.controller._updateInteractionState(
        cursor: widget.theme.cursorStyle,
      );
    }

    widget.onCanvasPanStateChanged?.call(isPanning);
  }
